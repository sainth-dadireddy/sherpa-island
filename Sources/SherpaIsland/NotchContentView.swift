...
struct NotchContentView: View {
    ...
    @AppStorage("AgentChat.voiceEnabled") private var agentChatVoice = true
    @AppStorage("AgentChat.notificationsEnabled") private var agentChatBanner = true
    @AppStorage("AgentChat.soundEnabled") private var agentChatSound = true
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 8) {
                Image(systemName: "speaker.wave.2")
                    .font(.system(size: 10))
                    .foregroundColor(agentChatVoice ? accent : .white.opacity(0.35))
                    .frame(width: 14)
                Text("Voice on new msg")
                    .font(.system(size: 11, design: .rounded))
                    .foregroundColor(.white.opacity(0.85))
                Spacer()
                Toggle("", isOn: $agentChatVoice)
                    .toggleStyle(.switch).controlSize(.mini).tint(accent).labelsHidden()
            }

            HStack(spacing: 8) {
                Image(systemName: "bell.badge")
                    .font(.system(size: 10))
                    .foregroundColor(agentChatBanner ? accent : .white.opacity(0.35))
                    .frame(width: 14)
                Text("Banner notification")
                    .font(.system(size: 11, design: .rounded))
                    .foregroundColor(.white.opacity(0.85))
                Spacer()
                Toggle("", isOn: $agentChatBanner)
                    .toggleStyle(.switch).controlSize(.mini).tint(accent).labelsHidden()
            }
            ...
            HStack(spacing: 8) {
                Image(systemName: "speaker.3")
                    .font(.system(size: 10))
                    .foregroundColor(agentChatSound ? accent : .white.opacity(0.35))
                    .frame(width: 14)
                Text("Tink sound")
                    .font(.system(size: 11, design: .rounded))
                    .foregroundColor(.white.opacity(0.85))
                Spacer()
                Toggle("", isOn: $agentChatSound)
                    .toggleStyle(.switch).controlSize(.mini).tint(accent).labelsHidden()
            }
        }
    }
